...
fn radix_sort_a(
    ...
    for (var entry_index = start_entry_index; entry_index < end_entry_index; entry_index += 1u) {
        ...
        if (in_frustum_sort(clip_space_pos.xyz)) {
            // Stable tie-break when float keys collide: subtract bounded index (no underflow).
            let sub = entry_index & 0x3FFu;
            key = key_distance - min(sub, key_distance);
        }
...
// Parallel exclusive prefix per histogram row (Hillis–Steele); one 256-thread workgroup per digit place.
...
fn radix_sort_c_scatter(
    ...
    // Step 2: Parallel exclusive prefix over per-bin counts (Hillis–Steele).
    var orig_i = 0u;
    if (tid < #{RADIX_BASE}u) {
